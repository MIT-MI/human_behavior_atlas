...
> 🗂️ **Repo layout:** training code lives under [`training/`](training/) — `training/sft/` (supervised fine-tuning) and `training/rl/` (GRPO). Evaluation utilities are in [`evaluation/`](evaluation/), extra docs in [`docs/`](docs/). The RL engine is the [`verl/`](verl/) submodule. See **[`docs/CODE_STRUCTURE.md`](docs/CODE_STRUCTURE.md)** for a full map.
>
