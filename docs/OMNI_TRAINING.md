# Omni SFT + GRPO/HARPO on the verl fork (`parquet_dataloader_omni_mirl`)
...
code but configured for the omni model, and adding the fork-only **GRPO** and **HARPO**
...
| RL algos | GRPO | **GRPO + HARPO** (HARPO is fork-only) |
...
│   └── rl/   # fork GRPO + HARPO (run_grpo.sh, run_harpo.sh, reward_function/, format_prompt/)
...
bash scripts/setup_verl.sh    # apply the HBA patches (binary-parquet loader + HARPO) to the pinned verl submodule
...
`human_behaviour_harpo.py` (HARPO, task-aware); prompt template
`format_prompt/default.jinja`.

> **Why the fork?** HARPO (`adv_estimator=harpo`) and the multimodal modality-batching
...
decreasing loss on an H20) with the omni model class selected by config. The GRPO/HARPO
