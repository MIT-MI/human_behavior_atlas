"""
Entry point for QA stage training.

Loads a frozen multi-head classification checkpoint, unfreezes only lm_head,
and trains with teacher-forced LM loss on the specified QA datasets.
"""
import os
import sys
import json
...
from trainer.qa_trainer import QAMultiHeadTrainer


def parse_parameters():
    parser = argparse.ArgumentParser(description='QA stage training (freeze base, train lm_head)')
    ...
    # QA-specific
    parser.add_argument('--qa_datasets', type=str, nargs='+',
                        help='Dataset names to treat as QA (LM loss, not classification)')
    parser.add_argument('--qa_loss_weight', type=float, default=1.0)
    ...
    params = {
        ...
        # QA-specific
        'qa_datasets':                  args.qa_datasets or [],
...
def main():
    ...
    print(f"[INFO] Initializing MultiHeadOmniClassifier with {params['num_classes']} classes")
    ...
    global_config = {
        ...
        # QA-specific
        'QA_DATASETS':                  params['qa_datasets'],
        'QA_LOSS_WEIGHT':               params['qa_loss_weight'],
    }

    trainer = QAMultiHeadTrainer(
